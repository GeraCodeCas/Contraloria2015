...
if(sOpcion.equals("SAVE")){
	...
	if(sCodigo.equals("0")){
		...
		oSeguridad.Bitacora(sUsuario, "Nuevo", "Anexo III.15", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
	} else {
		if (!sCodigo.equals("")){
	  		oAnexo.Update(java.lang.Integer.parseInt(sCodigo), iIdFuncionario, sIdentificacion, sExpediente, sPeriodo, sUbicacion, sDocumento);
			// PARA GUARDAR BITACORA
			oSeguridad.Bitacora(sUsuario, "Grabar", "Anexo III.15", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
		}else{
		// PARA GUARDAR BITACORA
		oSeguridad.Bitacora(sUsuario, "Grabar", "Anexo III.15 Estatus", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
		}
	}
	sCodigo = "";
	sOpcion = "";
...
if(sOpcion.equals("DELETE")){
	// PARA GUARDAR BITACORA
	oSeguridad.Bitacora(sUsuario, "Elimina", "Anexo III.15", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
...
if(sOpcion.equals("DELALL")){
	// PARA GUARDAR BITACORA
	oSeguridad.Bitacora(sUsuario, "Elimina", "Anexo III.15 Contenido", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
...
if(sOpcion.equals("UNDO")){
	// PARA GUARDAR BITACORA
	oSeguridad.Bitacora(sUsuario, "DeshaceCarga", "Anexo III.15", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
...
<html>
<head>
	<title>Entrega - Recepción : Anexo III.15 (Procesando...)</title>
</head>
<body>
	<form action="fAnexo3XV.jsp" name="forma" method="post" target="_parent">
