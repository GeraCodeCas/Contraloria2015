...
<html>
<head>
	<title>Entrega - Recepción : Anexo III.5.2.2 (Impresión)</title>
...
<tr>
	...
	<TR><TD class="TituloParrafoImpr">RELACIÓN DE PROGRAMAS TIPO PAQUETE</TD></TR>  
	</TABLE>
	<TABLE cellPadding="0" cellSpacing="0" width="100%" BORDER="1" BORDERCOLOR="white">
		<TR align="middle">
			<td class="TituloMenuImpr" width="200">SOFTWARE</TD>
			<td class="TituloMenuImpr" width="200">MARCA</TD>
			<td class="TituloMenuImpr" width="200">VERSIÓN</TD>
			<td class="TituloMenuImpr" width="200">LICENCIA</TD>
			<td class="TituloMenuImpr" width="50">DISCOS</TD>
			<td class="TituloMenuImpr" width="50">MANUALES</TD>
		</TR>
		<%
if(!sEstatus.equals("0")){
		...
		String vSoftware =oAnexo3V2II.GetSoftware(i)!=null?oAnexo3V2II.GetSoftware(i):"";
		//Primer Elemento que su contenido puede llevar a un salto de pagina
		iLenCol = vSoftware.length();
		iRowsXCol = 1;
		if(iLenCol > 27){
			if(iLenCol%27 == 0){
		...
		String vMarca = oAnexo3V2II.GetMarca(i)!=null?oAnexo3V2II.GetMarca(i):"";
		...
		if(iLenCol > 27){
			...
			for(int x=1;x<=(iRowsXCol);x++){
				int max = (27*x)<=iLenCol ? (27*x) : iLenCol;
				if( (x + iRowsAnt) <= iMaxRowXPag) {
					vMarca = vMarca + temp.substring((27*(x-1)), max) + "<br>";	}
				else {
					iRowCountTemp = iRowCountTemp + 1;
		...
		String vVersion = oAnexo3V2II.GetVersion(i)!=null?oAnexo3V2II.GetVersion(i):"";
		String vLicencia = oAnexo3V2II.GetLicencia(i)!=null?oAnexo3V2II.GetLicencia(i):"";
		// Elemento que su contenido puede llevar a un salto de pagina
		iLenCol = vLicencia.length();
		if(iLenCol > 27){
			if(iLenCol%27 == 0){
				iRowsXCol = iLenCol/27;
			}else{
			...
			String temp = vLicencia;
			vLicencia = "";
			iRowCountTemp = 0 ;
			for(int x=1;x<=(iRowsXCol);x++){
				int max = (27*x)<=iLenCol ? (27*x) : iLenCol;
				if( (x + iRowsAnt) <= iMaxRowXPag) {
					vLicencia = vLicencia + temp.substring((27*(x-1)), max) + "<br>";	}
				else {
					iRowCountTemp = iRowCountTemp + 1;
		...
		String vDiscos = oAnexo3V2II.GetDiscos(i)!=null?oAnexo3V2II.GetDiscos(i):"";
		String vManuales = oAnexo3V2II.GetManuales(i)!=null?oAnexo3V2II.GetManuales(i):"";
		...
		<TR valign="top" align="left" bgcolor="<%=sBackColor%>");">
			<TD class="ContenidoImprL"><%=vSoftware%></TD>
			<TD class="ContenidoImpr"><%=vMarca%></TD>
			<TD class="ContenidoImpr"><%=vVersion%></TD>
			<TD class="ContenidoImpr"><%=vLicencia%></TD>
			<TD class="ContenidoImpr" align="center"><%=vDiscos%></TD>
			<TD class="ContenidoImprR" align="center"><%=vManuales%></TD>
		</TR>
		<%
