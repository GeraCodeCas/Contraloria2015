...
<html>
<head>
	<title>Entrega - Recepción : Anexo I.2</title>
...
<FORM action="pAnexo1II.jsp" name="forma" method="post" >
	<input type="hidden" name="sOpcion" value="">
	<input type="hidden" name="sCodigo" value="<%=sCodigo%>"> 
	<TABLE align=center border=0 cellPadding=0 cellSpacing=0 width="100%">
		<TR>
			<TD  colspan="3" class="TituloParrafo" width="70%">&nbsp;&nbsp;ANEXO I.2 - RELACIÓN DE ACUERDOS, CONVENIOS, CONTRATOS Y DOCUMENTOS LEGALES</TD>
		</TR>
		<TR>
		<TD width="70%">
		...
		if( !sMensageEstatus.equals("") ) {
			...
			if(!bSoloLectura) { %>
				<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="Carga('<%=iIdFuncionario%>','<%=iIdAnexo%>', 'cAnexo1II.jsp');"><img alt="Carga Masiva" src="<%=sPath%>Images/Carga.jpg"></a>&nbsp;&nbsp;
				...
				<%}%>
				<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="imprimir(<%=iIdFuncionario%>, 'iAnexo1II.jsp');"><img src="<%=sPath%>Images/Impresora.jpg" ></a>&nbsp;&nbsp;
				<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="ayuda('../Help/aAnexo1II.htm');"><img src="<%=sPath%>Images/Ayuda.jpg" ></a></td> 
			<TD align=right>
	<%//AGREGADO POR VALIDACION ESTATUS 5
	oAnexo1II.GetAll(iIdFuncionario, iItemsPagina, iPagina); 
	long iAllCount = oAnexo1II.GetAllCount();
	if(!bSoloLectura) {
	...
	<!-- Etiqueta donde se aplican los encabezados de las tablas-->
		...
		<TR align="middle">
        	<TD colspan="2"></TD>
				...
				for(int j=0;j<oAnexo1II.GetTipoAll();j++){
					int oIdTipo	= oAnexo1II.GetIdTipoTipo(j);
					String oNombre 			= oAnexo1II.GetNombre(j);			 %>
					<option value="<%=oIdTipo%>"><%=oNombre%></option><%
				}%>
				</select></TD>
        	<TD>
				<INPUT class="txtBox" name="sNumero" id="sNumero" type="Text" size="20" value="" maxlength="255" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
				<INPUT class="txtBox" name="sFechaSuscripcion" id="sFechaSuscripcion" type="Text" size="9" maxlength="10" value="" readonly>&nbsp;<img src="<%=sPath%>Images/Fecha.gif" align="top" border=0 onclick="fPopCalendar(sFechaSuscripcion,sFechaSuscripcion)"></TD>
			<TD>
				<INPUT class="txtBox" name="sDuracion" id="sDuracion" type="Text" size="20" maxlength="50" value="" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
				<INPUT class="txtBox" name="sDescripcion" id="sDescripcion" type="Text" size="30" value="" maxlength="255" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
				<INPUT class="txtNumeric" name="sImporteTotal" id="sImporteTotal" type="Text" size="20" value="" maxlength="20" onkeypress="valida(this);" onpaste="return validaPasteNum(this);"></TD>
			<TD>
				<INPUT class="txtBox" name="sEntidad" id="sEntidad" type="Text" size="20" value="" maxlength="255" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
...
<%		
		...
		for(int i=0;i<oAnexo1II.GetAll(iIdFuncionario, iItemsPagina, iPagina);i++){
		...
		<TR align="left" bgcolor="<%=sBackColor%>">
			...
			<TD><select class="cmbBox" name="sTipo"><%
				for(int j=0;j<oAnexo1II.GetTipoAll();j++){
					int oIdTipo	= oAnexo1II.GetIdTipoTipo(j);
					String oNombre 			= oAnexo1II.GetNombre(j);			 %>
					<option value="<%=oIdTipo%>" <%=java.lang.Integer.parseInt(vIdTipo)==oIdTipo?" selected":""%>><%=oNombre%></option><%
				}%>
			</TD>
			<TD>
				<INPUT class="txtBox" name="sNumero" id="sNumero" type="Text" size="10" value="<%=vNumero%>" maxlength="255" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
				<INPUT class="txtBox" name="sFechaSuscripcion" id="sFechaSuscripcion" type="Text" size="9" maxlength="10" value="<%=vFechaSuscripcion%>" readonly>&nbsp;<img src="<%=sPath%>Images/Fecha.gif" align="top" border=0 onclick="fPopCalendar(sFechaSuscripcion,sFechaSuscripcion)"></TD>
			<TD>
				<INPUT class="txtBox" name="sDuracion" id="sDuracion" type="Text" size="4" value="<%=vDuracion%>" maxlength="50" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
				<INPUT class="txtBox" name="sDescripcion" id="sDescripcion" type="Text" size="30" value="<%=vDescripcion%>" maxlength="255" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
				<INPUT class="txtNumeric" name="sImporteTotal" id="sImporteTotal" type="Text" size="10" value="<%=vImporteTotal%>"  maxlength="20" onkeypress="valida(this);" onpaste="return validaPasteNum(this);"></TD>
			<TD>
				<INPUT class="txtBox" name="sEntidad" id="sEntidad" type="Text" size="20" value="<%=vEntidad%>" maxlength="255" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD>
		...
		<TR align="left" bgcolor="<%=sBackColor%>" 
			...
			<TD align="center">&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vIdTipo%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vNumero%></a></TD>
			<TD align="center">&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vFechaSuscripcion%></a></TD>
			<TD align="center">&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vDuracion%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vDescripcion%></a></TD>
			<TD align="right">&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%try{out.println(myFormatter.format(new Double(!vImporteTotal.equals("")?vImporteTotal:"0")));}catch(Throwable e){System.out.println("fAnexo1ii.vImporteTotal: " + e + " " + vImporteTotal); out.println(vImporteTotal);} %></a>&nbsp;&nbsp;</TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vEntidad%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=vObservaciones%></a></TD>
		</TR><%
			}
		}
%>
	<TR><td colspan="2" ></td><TD colspan="3" class="ContenidoParrafo">TIPO:</TD><td colspan="5"></td></TR>
	<TR><td colspan="2" ></td><TD colspan="3" class="ContenidoParrafo">1 = Acuerdos </TD><td colspan="5" ></td></TR>
	<TR><td colspan="2" ></td><TD colspan="3" class="ContenidoParrafo">2 = Convenios </TD><td colspan="5" ></td></TR>
	<TR><td colspan="2" ></td><TD colspan="3" class="ContenidoParrafo">3 = Contratos </TD><td colspan="5" ></td></TR>
	<TR><td colspan="2" ></td><TD colspan="3" class="ContenidoParrafo">4 = Otros Documentos Legales</TD><td colspan="5" ></td></TR>
</TABLE>
<%if(iTotalPaginas>1){%>
	<TABLE cellPadding="0" cellSpacing="0" width="100%" BORDER="1" BORDERCOLOR="white">
		<TR><TD width="50%">&nbsp;</TD>
