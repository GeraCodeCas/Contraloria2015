...
if(sOpcion.equals("SAVE")){
	...
	if(sCodigo.equals("0")){
		...
		oSeguridad.Bitacora(sUsuario, "Nuevo", "Anexo III.13", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
	} else {
		if (!sCodigo.equals("")){
			...
			// PARA GUARDAR BITACORA
			oSeguridad.Bitacora(sUsuario, "Grabar", "Anexo III.13", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
		}else{
		// PARA GUARDAR BITACORA
		oSeguridad.Bitacora(sUsuario, "Grabar", "Anexo III.13 Estatus", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
		}
	}
	sCodigo = "";
	sOpcion = "";
...
if(sOpcion.equals("DELETE")){
	// PARA GUARDAR BITACORA
	oSeguridad.Bitacora(sUsuario, "Elimina", "Anexo III.13", request.getRemoteAddr(), request.getRemoteHost(), request.getHeader("User-Agent"));
...
<html>
<head>
	<title>Entrega - Recepción : Anexo III.13 (Procesando...)</title>
</head>
<body>
	<form action="fAnexo3XIII.jsp" name="forma" method="post" target="_parent">
