...
<html>
<head>
	<title>Entrega - Recepción : Anexo III.16 (Impresión)</title>
...
<tr>
	...
	<TABLE cellPadding="0" cellSpacing="0" width="100%" BORDER="1" BORDERCOLOR="white">
		<TR><TD class="TituloParrafoImpr">RELACIÓN DE ARCHIVO MUERTO</TD></TR>  
	</TABLE>
	<TABLE cellPadding="0" cellSpacing="0" width="100%" BORDER="1" BORDERCOLOR="white">
		<TR align="middle">
			<td class="TituloMenuImpr" width="174">IDENTIFICACIÓN DEL ARCHIVO</TD>
			<td class="TituloMenuImpr" width="237">EXPEDIENTE</TD>
			<td class="TituloMenuImpr" width="163">PERIODO COMPRENDIDO</TD>
			<td class="TituloMenuImpr" width="163">UBICACIÓN</TD>
			<td class="TituloMenuImpr" width="163">DOCUMENTOS O PLANOS</TD>
		</TR>
<%
if(!sEstatus.equals("0")){
		...
		for(int i=0;i<oAnexo.GetAll(iIdFuncionario);i++){
			...
			long 	iIdRenglon			= oAnexo.GetIdRenglon(i)!=-1?			oAnexo.GetIdRenglon(i):0;
			...
			if(iLenCol > 25){
				if(iLenCol%25 == 0){
					iRowsXCol = iLenCol/25;
				}else{
					iRowsXCol = (iLenCol/25)+1;
				}
				String temp = sIdentificacion;
				sIdentificacion = "";
				iRowCountTemp = 0 ;
				for(int x=1;x<=(iRowsXCol);x++){
					int max = (25*x)<=iLenCol ? (25*x) : iLenCol;
					if( (x + iRowsAnt) <= iMaxRowXPag) {
						sIdentificacion = sIdentificacion + temp.substring((25*(x-1)), max) + "<br>";
						}
					else {
						iRowCountTemp = iRowCountTemp + 1;
						vTempCampo1 = vTempCampo1 + temp.substring((25*(x-1)), max) + "<br>";
						}
				}
			...
			if(iLenCol > 34){
				if(iLenCol%34 == 0){
					iRowsXCol = iLenCol/34;
				}else{
					iRowsXCol = (iLenCol/34)+1;
				}
				...
				iRowCountTemp = 0 ;
				for(int x=1;x<=(iRowsXCol);x++){
					int max = (34*x)<=iLenCol ? (34*x) : iLenCol;
					if( (x + iRowsAnt) <= iMaxRowXPag) {
						sExpediente = sExpediente + temp.substring((34*(x-1)), max) + "<br>";	}
					else {
						iRowCountTemp = iRowCountTemp + 1;
						vTempCampo2 = vTempCampo2 + temp.substring((34*(x-1)), max) + "<br>";	}
				}
				iRowMaxTemp = iRowMaxTemp >= iRowCountTemp ? iRowMaxTemp : iRowCountTemp ;
			}else{
				iRowsXCol = 1;
			...
			if(iLenCol > 23){
				if(iLenCol%23 == 0){
					iRowsXCol = iLenCol/23;
				}else{
					iRowsXCol = (iLenCol/23)+1;
				}
				if (iRowsXCol > iRowsMax){
				    iRowsCount = iRowsXCol - iRowsMax + iRowsCount;
					iRowsMax = iRowsXCol;
				...
				iRowCountTemp = 0 ;
				for(int x=1;x<=(iRowsXCol);x++){
					int max = (23*x)<=iLenCol ? (23*x) : iLenCol;
					if( (x + iRowsAnt) <= iMaxRowXPag) {
						sPeriodo = sPeriodo + temp.substring((23*(x-1)), max) + "<br>";	}
					else {
						iRowCountTemp = iRowCountTemp + 1;
						vTempCampo3 = vTempCampo3 + temp.substring((23*(x-1)), max) + "<br>";	}
				}
				iRowMaxTemp = iRowMaxTemp >= iRowCountTemp ? iRowMaxTemp : iRowCountTemp ;
			}else{
				iRowsXCol = 1;
			...
			if(iLenCol > 23){
				...
				for(int x=1;x<=(iRowsXCol);x++){
					int max = (23*x)<=iLenCol ? (23*x) : iLenCol;
					if( (x + iRowsAnt) <= iMaxRowXPag) {
						sUbicacion = sUbicacion + temp.substring((23*(x-1)), max) + "<br>";	}
					else {
						iRowCountTemp = iRowCountTemp + 1;
						vTempCampo4 = vTempCampo4 + temp.substring((23*(x-1)), max) + "<br>";	}
				}
				iRowMaxTemp = iRowMaxTemp >= iRowCountTemp ? iRowMaxTemp : iRowCountTemp ;
			}else{
				iRowsXCol = 1;
			...
			if(iLenCol > 23){
				...
				for(int x=1;x<=(iRowsXCol);x++){
					int max = (23*x)<=iLenCol ? (23*x) : iLenCol;
					if( (x + iRowsAnt) <= iMaxRowXPag) {
						sDocumento = sDocumento + temp.substring((23*(x-1)), max) + "<br>";	}
					else {
						iRowCountTemp = iRowCountTemp + 1;
						vTempCampo5 = vTempCampo5 + temp.substring((23*(x-1)), max) + "<br>";	}
				}
				iRowMaxTemp = iRowMaxTemp >= iRowCountTemp ? iRowMaxTemp : iRowCountTemp ;
			}else{
				iRowsXCol = 1;
