...
<html>
<head>
	<title>Entrega - Recepción : Anexo III.5.2</title>
...
<FORM action="pAnexo3V2.jsp" name="forma" method="post" >
	<input type="hidden" name="sOpcion" value="">
	<input type="hidden" name="sCodigo" value="<%=sCodigo%>"> 
	<TABLE align=center border=0 cellPadding=0 cellSpacing=0 width="100%">
		<TR>
			<TD colspan="3" class="TituloParrafo" width="100%">&nbsp;&nbsp;ANEXO III.5.2 - RELACIÓN DE EQUIPO DE COMPUTO</TD>
		</TR>
		<TR>
		...
		if( !sMensageEstatus.equals("") ) {
			...
			<%//AGREGADO POR VALIDACION ESTATUS
			if(!bSoloLectura) { %>
			<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="Carga('<%=iIdFuncionario%>','<%=iIdAnexo%>', 'cAnexo3V2.jsp');"><img src="<%=sPath%>Images/Carga.jpg" ></a>&nbsp;&nbsp;
			...
			<%}%>
			<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="imprimir('<%=iIdFuncionario%>', 'iAnexo3V2.jsp');"><img src="<%=sPath%>Images/Impresora.jpg" ></a>&nbsp;&nbsp;
			<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="ayuda('../Help/aAnexo3V2.htm');"><img src="<%=sPath%>Images/Ayuda.jpg" ></a></td> 
			<TD align=right>
			<%//AGREGADO POR VALIDACION ESTATUS
			if(!bSoloLectura) {
...
<%		VMatrix vArchivoA;
	...
	<%if(sOpcion.equals("NEW")){%>
		<TR align="middle">
			...
			<TD><select class="cmbBox" name="sTipo"><%
				for(int j=0;j<oAnexo3V2.GetTipoAll();j++){%>
					<option value="<%=oAnexo3V2.GetIdTipo(j)%>"><%=oAnexo3V2.GetDesTipo(j)%></option><%
				}%>
				</select></TD>
		</TR><%
	}
	if(!sEstatus.equals("0")){ // DETALLE DE ANEXO	
		int intContador = 0;
		String sBackColor = "";
		oAnexo3V2.GetAll(iIdFuncionario, iItemsPagina, iPagina); 
		long iAllCount = oAnexo3V2.GetAllCount();
		...
		for(int i=0;i<oAnexo3V2.GetAll(iIdFuncionario, iItemsPagina, iPagina);i++){
		...
		<TR align="left" bgcolor="<%=sBackColor%>">
        	<TD>&nbsp;</TD>
        	<TD><INPUT class="txtBox" name="sClave"			id="sClave"			type="Text" size="15" maxlength="20" value="<%=oAnexo3V2.GetClave(i)!=null?oAnexo3V2.GetClave(i):""%>" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD><INPUT class="txtBox" name="sNumFactura"	id="sNumFactura"	type="Text" size="15" maxlength="20" value="<%=oAnexo3V2.GetNumFactura(i)!=null?oAnexo3V2.GetNumFactura(i):""%>" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD><INPUT class="txtBox" name="sFecha"			id="sFecha"			type="Text" size="10" maxlength="10" value="<%=oAnexo3V2.GetFecha(i)!=null?oAnexo3V2.GetFecha(i):""%>"
					readonly>&nbsp;<img src="../Images/Fecha.gif" align="top" border=0 onclick="fPopCalendar(sFecha,sFecha)"></TD>
			<TD><INPUT class="txtBox" name="sConcepto"		id="sConcepto"		type="Text" size="40" maxlength="50" value="<%=oAnexo3V2.GetConcepto(i)!=null?oAnexo3V2.GetConcepto(i):""%>" onKeyPress="isInValid();" onpaste="validaPasteStr(this);"></TD>
			<TD><INPUT class="txtNumeric" name="sImporte"		id="sImporte"		type="Text" size="15" maxlength="20" onkeypress="valida(this);" onpaste="return validaPasteNum(this);" value="<%=oAnexo3V2.GetImporte(i)!=null?oAnexo3V2.GetImporte(i):""%>"></TD>
			<TD><select class="cmbBox" style="font-size:9px;" name="sTipo"><%
				for(int j=0;j<oAnexo3V2.GetTipoAll();j++){%>
					<option value="<%=oAnexo3V2.GetIdTipo(j)%>" <%=oAnexo3V2.GetIdTipo(j).equals(oAnexo3V2.GetTipo(i))?" selected":""%>><%=oAnexo3V2.GetDesTipo(j)%></option><%
				}%>
				</select></TD>
		</TR><%
			} else {%>
		<TR align="left" bgcolor="<%=sBackColor%>" onMouseOver = "MouseDentro(this);" onMouseOut  = "MouseFuera(this,'<%=sBackColor%>');">
			...
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=oAnexo3V2.GetClave(i)!=null?oAnexo3V2.GetClave(i):""%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=oAnexo3V2.GetNumFactura(i)!=null?oAnexo3V2.GetNumFactura(i):""%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=oAnexo3V2.GetFecha(i)!=null?oAnexo3V2.GetFecha(i):""%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=oAnexo3V2.GetConcepto(i)!=null?oAnexo3V2.GetConcepto(i):""%></a></TD>
			<TD align="right">&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=myFormatter.format(new Double(oAnexo3V2.GetImporte(i)!=null?oAnexo3V2.GetImporte(i):"0"))%></a></TD>
			<TD>&nbsp;<a onmouseover="this.style.cursor='hand'" style="cursor:normal" onclick="edit(<%=vIdRenglon%>);"><%=oAnexo3V2.GetDesTipo(oAnexo3V2.GetTipoPos(oAnexo3V2.GetTipo(i)))%></a></TD>
		</TR><%
			}
		}%>
	</TABLE><%
